...
def connect_twitter(access_details):
 ...
 # for each combination we need to query
  ...
  tweet_id,tweet_count,search_request_count = 0,0,0
 ...
 #search = api.GetSearch(term=access_details['hashtags'], geocode='20.593684020,78.962880078,1km',lang='en', result_type='recent', count=10, max_id='')
  ...
  logger.debug("Total search request [%s], Tweet count [%s] in (seconds) CPU time [%s], Operation time [%s]", search_request_count,tweet_id,(time.clock() - cpu_start_time), (time.time() - operation_start_time))
